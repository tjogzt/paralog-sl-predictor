...
**核心发现（详见论文全文）：**
- **金标准评估（证据分级、引用逐条核验）：** 完整 12 对策展集 AUROC = 0.794；去除 2 对 DepMap 来源标签后 0.833；仅用 DepMap 之前证据 0.900；方向严格重标后 0.767。两条可评估的 Tier A 记录（方向性外部实验证据）均排在全部未标记对照之上
- **DD vs. 已发表方法（背景参照，CV3 类框架）：** 无需训练 AUROC = 0.794；已发表 CV3 最佳 0.790（SLMGAE）——评估框架类似但不完全相同
