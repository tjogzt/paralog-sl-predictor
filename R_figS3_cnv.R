# FigS3 — CNV Independence Analysis (R, real DepMap data)
...
ggsave(file.path(OUT_DIR, "FigS3_CNV_Independence.pdf"), p,
       width = 180, height = h_mm, units = "mm", device = cairo_pdf)
ggsave(file.path(OUT_DIR, "FigS3_CNV_Independence.svg"), p,
       width = 180, height = 180, units = "mm", device = svglite::svglite)
ggsave(file.path(OUT_DIR, "FigS3_CNV_Independence.tiff"), p,
       width = 180, height = 180, units = "mm", device = ragg::agg_tiff, dpi = 600)
message(sprintf("FigS3_CNV_Independence.pdf (180×%dmm) ✓", h_mm))
