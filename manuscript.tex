...
\textbf{Results:} We introduce Delta Dependency (DD), a single interpretable metric that measures the dependency shift on a paralog gene between driver-mutant and wild-type cell lines in DepMap (1,208 lines, 23 solid tumor types). In the pre-specified primary evaluation --- a lineage-level benchmark on twelve curated, evidence-tiered paralog-SL pairs --- DD achieved AUROC~$=$~0.794 (0.833 after excluding two pairs with DepMap-derived labels). In a pair-level benchmark on the same 77 test pairs, DD alone outperformed four standard multi-feature classifiers under leave-one-pair-out cross-validation. Exploratory analyses across seven CPTAC cohorts ($n=771$) detected protein-level paralog co-variation invisible at the RNA level. Dependency-window scoring prioritized ARID1A$\rightarrow$ARID1B (DWS~$=$~4.13) as the leading selective candidate, suggesting a hypothesis-generating biomarker strategy centered on truncating ARID1A mutations.
...
\textbf{a}, DD AUROC across 17 evaluable solid tumor types (red: AUROC~$>$~0.7; blue: 0.5--0.7; gray: $<$~0.5). Dashed line, random classifier (0.5). Bar labels show $n$ tested pairs. Sample sizes and full lineage names in Supplementary Table~S1.
...
\textbf{b}, Therapeutic window classification. Bubble size, mean DWS; color, selectivity tier. HIGH\_SELECTIVITY requires selectivity~$>$~0.15 and DWS~$>$~1.0; NF1$\rightarrow$RASA2 has high DWS (3.31) but near-zero selectivity (0.003), hence MODERATE.
...
\textbf{d}, Composite preclinical prioritization score ranking the top 10 candidates. Scores integrate dependency window score (0.40), selectivity (0.30), structural similarity (0.15), and druggability (0.15). ARID1A$\rightarrow$ARID1B ranks first (0.817).}
