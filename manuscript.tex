...
\textbf{Results:} We introduce Delta Dependency (DD), a single interpretable metric that directly measures the dependency shift on a paralog gene between driver-mutant and wild-type cell lines in DepMap (1,208 lines, 23 solid tumor types). On a curated set of twelve literature-derived paralog-SL pairs with tiered experimental evidence, DD achieved AUROC~$=$~0.794 (0.833 after excluding two pairs with DepMap-derived labels) and outperformed four standard multi-feature classifiers under leave-one-pair-out cross-validation. Across seven CPTAC cohorts ($n=771$), we detected protein-level paralog co-variation that was invisible at the RNA level. In endometrial tumors, ARID1B protein was 55\% higher in ARID1A-mutant than wild-type samples ($p=0.082$), consistent with a stoichiometric compensation model. Therapeutic window analysis prioritized ARID1A$\rightarrow$ARID1B (DWS~$=$~4.13) as the leading selective candidate, with a biomarker-enriched trial design for MSS tumors harboring truncating ARID1A mutations.
