# FigS4 — CPTAC Per-Cohort Scatter Plots (R, real UCEC data)
...
# ── MAIN ──
message("=== FigS4 (Real UCEC CPTAC Data) ===")
...
ggsave(file.path(OUT_DIR, "FigS4_panel_a.pdf"), pa, width = PANEL_W, height = PANEL_H, units = "mm", device = cairo_pdf)
ggsave(file.path(OUT_DIR, "FigS4_panel_b.pdf"), pb, width = PANEL_W, height = PANEL_H, units = "mm", device = cairo_pdf)
ggsave(file.path(OUT_DIR, "FigS4_panel_c.pdf"), pc, width = PANEL_W, height = PANEL_H, units = "mm", device = cairo_pdf)
...
ggsave(file.path(OUT_DIR, "FigS4_CPTAC_PerCohort.pdf"), p,
       width = 180, height = 60, units = "mm", device = cairo_pdf)
ggsave(file.path(OUT_DIR, "FigS4_CPTAC_PerCohort.svg"), p,
       width = 180, height = 60, units = "mm", device = svglite::svglite)
ggsave(file.path(OUT_DIR, "FigS4_CPTAC_PerCohort.tiff"), p,
       width = 180, height = 60, units = "mm", device = ragg::agg_tiff, dpi = 600)
message("FigS4_CPTAC_PerCohort.pdf (180×60mm) ✓")
