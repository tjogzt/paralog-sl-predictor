...
fig = plt.figure(figsize=(14.2, 8.2), dpi=300)
...
def box(x, y, w, h, color, fill):
    ax.add_patch(FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.6,rounding_size=1.2",
                                linewidth=2.2, edgecolor=color, facecolor=fill))


def arrow(x1, x2, y):
    ax.add_patch(FancyArrowPatch((x1, y), (x2, y), arrowstyle="-|>",
                                 mutation_scale=22, linewidth=2.2, color=NAVY))


# ── Top row: pipeline ──────────────────────────────────────────
T, H = 62, 22  # bottom y, height of top boxes
box(3, T, 20, H, BLUE, "#e9f4fc")
fig.text(0.135, 0.79, "DepMap 26Q1", ha="center", fontsize=14, fontweight="bold", color=NAVY)
for i, s in enumerate(["1,208 cell lines × 18,531 genes",
                       "HGNC 66,595 paralog pairs",
                       "12 gold-standard SL pairs"]):
    fig.text(0.135, 0.745 - i * 0.038, s, ha="center", fontsize=10.5, color="#34495e")

box(27.5, T, 20, H, GREEN, "#e8f8ee")
fig.text(0.38, 0.79, "D = μ(CERES|WT)", ha="center", fontsize=14, fontweight="bold", color=NAVY)
fig.text(0.38, 0.745, "− μ(CERES|MUT)", ha="center", fontsize=14, fontweight="bold", color=NAVY)
fig.text(0.38, 0.685, "Univariate", ha="center", fontsize=10.5, color=GREEN)
fig.text(0.38, 0.647, "No training needed", ha="center", fontsize=10.5, color=GREEN)

box(52, T, 20, H, RED, "#fdeceb")
fig.text(0.625, 0.79, "Priority Candidates", ha="center", fontsize=14, fontweight="bold", color=NAVY)
fig.text(0.625, 0.745, "ARID1A→ARID1B", ha="center", fontsize=11.5, fontweight="bold", color=RED)
fig.text(0.625, 0.707, "EP300→CREBBP", ha="center", fontsize=10.5, color="#34495e")
fig.text(0.625, 0.669, "KRAS→HRAS  PIK3R1→CRKL", ha="center", fontsize=10, color=GRAY)
...
for num, name, color, lines in layers:
    ...
    fig.text(cx, (Y0 + HH - 13.5) / 100, name, ha="center", fontsize=12.5,
