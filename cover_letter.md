...
1. **DD outperforms multi-feature classifiers in a rigorous head-to-head benchmark.** On a curated set of 12 literature-derived paralog-SL pairs with tiered, citation-verified experimental evidence, DD achieves AUROC = 0.794 — robust to label quality (0.833 excluding two DepMap-derived pairs; 0.767 under a direction-strict relabelling) — and both lineage-evaluable pairs with direct directional experimental evidence rank above all unlabeled controls. In a rigorous head-to-head benchmark on the same 77 pairs (8 known positives), DD alone (AUROC = 0.736) outperforms four multi-feature classifiers — logistic regression (0.632), random forest (0.629), SVM-Linear (0.699), and SVM-RBF (0.563) — under leave-one-pair-out cross-validation. A DD + sequence-identity (≥30%) filter reaches AUROC = 1.000 on a high-identity subset (p = 0.0048 by exact permutation).
...
4. **Clinical stratification and safety analysis.** Using the official DepMap MSIsensor2 annotation, MSI-H subgroups show paralog-SL signal at least as strong as MSS subgroups in both colorectal and endometrial lineages, mutation type modulates compensation strength (truncating > missense for well-characterized TSGs), and a quantitative therapeutic window framework nominates ARID1A→ARID1B (DWS = 4.13) as the leading selective candidate, with a biomarker-enriched trial design centered on truncating ARID1A mutations.
