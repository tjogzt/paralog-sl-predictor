...
5. **Every number is one-command reproducible.** All analyses use publicly available DepMap and CPTAC data, and the method requires only subtraction: no GPU, no model training. Beyond the open-source R package (`paralogSL` v1.0.2) and Python pipeline, every quantitative claim is verified by a layered audit system: per-script claim checks for headline metrics (16/16 verified), classifier benchmarks, and covariate-adjusted regressions (10/10 verified), plus a master closed-loop audit that independently recomputes **165 numeric claims** — every AUROC, p-value, sample size, and effect estimate across both the main text and the supplementary information — directly from the analysis artifacts and fails loudly on any mismatch. Every figure is generated by a script from the same artifacts (no manual edits; the few stochastic label-placement algorithms are seeded for reproducibility), and both documents passed a full-page visual review (page-by-page contact sheets included in the repository). A single entry point (`verify_all.sh`) runs all audits plus the test suite (31 tests) in under one minute. The submission-version code is archived on Zenodo (https://doi.org/10.5281/zenodo.21502030; R package: https://doi.org/10.5281/zenodo.21502113).
