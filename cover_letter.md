...
4. **Clinical stratification and safety analysis.** Using the official DepMap MSIsensor2 annotation, MSI-H subgroups show paralog-SL signal at least as strong as MSS subgroups in both colorectal and endometrial lineages, mutation type modulates compensation strength (truncating > missense for the leading TSG pair), and a quantitative therapeutic window framework nominates ARID1A→ARID1B (DWS = 2.82) as the leading selective candidate, with a biomarker-enriched trial design centered on truncating ARID1A mutations.
